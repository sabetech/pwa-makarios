...
import WeekdayServices from '../../page_partials/Services/Weekday';
...
const Services = () => {
    ...
    return (<>
        <MyNavBar currentPage="Services" rightNode={<Button fill='none' onClick={() => navigate("/dashboard")}><AiFillHome color='white' size={20} /></Button>} />
        ...
        >
             ...
             <Tabs.Tab title="Microchurch Services" key="-1">
                  <Button block color='primary' fill='solid' size='large' onClick={() => navigate('/services/microchurch/form')  }>
                    Fill Microchurch Service Form
                </Button>
                <List style={{paddingLeft: 0, marginTop: 20}}>
                {
                    services?.filter((service) => service.service_type_id === SERVICE_TYPES.MICRO_CHURCH).map((service) => (
                        <List.Item
                        key={service.id}
                        prefix={
                          <Image
